...
from network import direct_future_prediction
...
if __name__ == "__main__":
    ...
    game = initialize_vizdoom("maps/config/health_gathering.cfg", True)
    game.new_episode()
    ...
    agent.model = direct_future_prediction(state_size, measurement_size, goal_size, action_size, len(timesteps),
                                           agent.learning_rate)
    ...
    while not game.is_episode_finished():
        ...
        if t % 10000 == 0:
            ...
            if not os.path.exists("savefiles/direct-future"):
                print("Creating model directory")
                os.makedirs("savefiles/direct-future")
            print("Saving model to: savefiles/direct-future/dfp.h5")
            agent.model.save("savefiles/direct-future/dfp.h5", overwrite=True)
        ...
        if is_terminated:
            ...
            if GAME % agent.stats_window_size == 0 and t > agent.observe:
                ...
                if not os.path.exists("statistics/direct-future"):
                    print("Creating statistics directory")
                    os.makedirs("statistics/direct-future")

                with open("statistics/direct-future/dfp_stats.txt", "w") as stats_file:
